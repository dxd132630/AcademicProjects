...
public class MCP_TAS {
	...
	public MCP_TAS(int No_of_threads,int No_of_cores, int LBound, int UBound,int Initial_mean,int Step_up_for_mean_time, double lambda,int No_of_hits_to_CS,int No_of_iterations){
		...
		if(args.length == 0){
			...
			if(pow_of_two(No_of_threads)){
				...
				for(int i =LBound ; i<=UBound;i=i+Step_up_for_mean_time ){
					...
					new MCP_TAS(No_of_threads,No_of_cores, LBound, UBound,current_mean,Step_up_for_mean_time, lambda,No_of_hits_to_CS, No_of_iterations).test();
				}
